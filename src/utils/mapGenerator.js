...
export function generateMap(seed) {
  ...
  return Array.from({ length: 30 }, (_, r) =>
    ...
    Array.from({ length: 30 }, (_, c) => {
      ...
      // - Weight 9: Very difficult terrain (val > 0.6)
      // - Weight 8: Difficult terrain (val > 0.475)
      // - Weight 7: Hard terrain (val > 0.35)
      // - Weight 6: Moderate-hard terrain (val > 0.225)
      // - Weight 5: Moderate terrain (val > 0.1)
      // - Weight 4: Easy-moderate terrain (val > -0.05)
      ...
      // with all weights from 1-9 represented for maximum variety.
      if (val > 0.6)   return { weight: 9 };
      if (val > 0.475) return { weight: 8 };
      if (val > 0.35)  return { weight: 7 };
      if (val > 0.225) return { weight: 6 };
      if (val > 0.1)   return { weight: 5 };
      if (val > -0.05) return { weight: 4 };
